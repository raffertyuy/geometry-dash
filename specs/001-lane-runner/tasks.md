...
- [X] T005 [P] Add ESLint flat config (ESLint 9) with `no-restricted-imports` rules per `contracts/module-contracts.md` (initially: Phaser banned outside `renderer/` and `phaser/`; cross-module imports must use `index.ts`). File: `eslint.config.js` (modern flat config instead of legacy `.eslintrc.cjs`). **Pivot 2026-05-16 evening: rule swapped Phaser->Three.js, and the allowed folders are now `renderer/` and `game/`.**
...
- [X] T011 Create `src/shared/config.ts` exporting `LANES`, `LANE_X`, `RUN_SPEED_UNITS_PER_SEC`, `LANE_SWITCH_DURATION_MS=200`, `SWIPE_MIN_HORIZONTAL_PX=30`, `SWIPE_MAX_DURATION_MS=500`, `SWIPE_HORIZONTAL_DOMINANCE=2`, `INPUT_COALESCE_WINDOW_MS=50`, and `DEBUG` (parses `?debug=1` once at module load, defaults `false` in non-browser env). File: `src/shared/config.ts`. **Pivot 2026-05-16 evening: `LANE_X` redefined to 3D world units `{ left: -2, centre: 0, right: 2 }`; `RUN_SPEED_UNITS_PER_SEC` changed from 200 (logical px/s) to 6 (world units/s); `LOGICAL_WIDTH` and `LOGICAL_HEIGHT` removed since Three.js uses 3D world space, not a fixed-resolution canvas.**
...
- [X] T014 [P] [US1] Write unit tests for `input-adapter` keyboard path covering: `ArrowLeft` and `'a'` emit direction `'left'`; `ArrowRight` and `'d'` emit `'right'`; unrecognised keys are ignored; `KeyboardEvent.repeat === true` events are dropped; two same-direction keyboard events within `INPUT_COALESCE_WINDOW_MS` produce one normalised `InputEvent`. Use the injected `now` clock from `InputAdapterDeps` for deterministic timestamps. File: `src/input-adapter/input-adapter.test.ts`
- [X] T015 [P] [US1] Write an integration test asserting the cross-module flow: an input event flows through `input-adapter.emit` into `applyInput` (mocked emit collector), `tickPlayer(200)` advances animation, `tickWorld(200)` advances distance, and the renderer would be called with the resulting state. Uses real `lane-state` + `runner-engine` modules; mocks renderer. File: `tests/integration/lane-switch-flow.test.ts`

### Implementation for User Story 1

- [X] T016 [P] [US1] Implement the lane-state module with the public API in `contracts/module-contracts.md` §`lane-state/`: `createPlayerState`, `applyInput`, `tickPlayer`, `adjacentLane`. Emit `console.debug` events for `lane_change_requested`, `lane_change_applied`, `lane_change_clamped`, `lane_change_buffered`. File: `src/lane-state/lane-state.ts` and `src/lane-state/index.ts`
- [X] T017 [P] [US1] Implement the runner-engine module with the public API in `contracts/module-contracts.md` §`runner-engine/`: `createWorldState`, `startRun`, `pauseRun`, `resumeRun`, `tickWorld`. Emit `console.debug` events for `run_started`, `run_paused`, `run_resumed`. File: `src/runner-engine/runner-engine.ts` and `src/runner-engine/index.ts`
- [X] T018 [P] [US1] Implement the input-adapter module (keyboard path only) with the public API in `contracts/module-contracts.md` §`input-adapter/`: `createInputAdapter({ now, emit })`, `handleKeyDown`. Recognise `ArrowLeft`/`a`/`A` and `ArrowRight`/`d`/`D`. Drop `repeat === true`. Enforce the 50 ms cross-source coalesce window. File: `src/input-adapter/input-adapter.ts` and `src/input-adapter/index.ts`
- [X] T019 [US1] Make all unit tests from T012, T013, T014 pass. Iterate on the implementation files from T016, T017, T018 only until green. **Blocked by**: T012, T013, T014, T016, T017, T018.
- [X] T020 [US1] Make the integration test from T015 pass. **Blocked by**: T015, T016, T017, T018, T019 (each unit test must be green individually before the integration test runs).
- [~] T021 [P] [US1] **PIVOTED 2026-05-16 evening (Phaser->Three.js).** Implement `src/renderer/three-renderer.ts`: instantiates a Three.js `Scene`, perspective camera (behind + above the player), `WebGLRenderer` bound to the canvas, ambient + directional lights, a ground plane, two thin lane dividers, a pool of scrolling "rungs" recycled when they pass the camera, and the player mesh (cube for now). `draw(player, world)` maps `currentLane` + `targetLane` + `easeOutCubic(animProgress)` to `player.position.x` using `LANE_X`, advances rung Z positions by the distance delta, and calls `renderer.render(scene, camera)`. `resize(w, h)` updates `camera.aspect` and `renderer.setSize`. `destroy()` disposes the WebGL renderer. Re-export via `src/renderer/index.ts`. File: `src/renderer/three-renderer.ts` and `src/renderer/index.ts`.
- [~] T022 [P] [US1] **PIVOTED.** Implement `src/renderer/debug-overlay.ts` as a DOM-based overlay: when `DEBUG === false`, returns a no-op object. When `DEBUG === true`, toggles `.hidden` on the given `<div id="debug-overlay">` element and writes a multiline `lane / animProgress / speed / distance / lastInput` text. The DOM element itself lives in `index.html`. File: `src/renderer/debug-overlay.ts`.
- [~] T023 [P] [US1] **PIVOTED. (Three.js does not have a pre-shipped game-config equivalent; this task is rolled into T026.)** N/A.
- [~] T024 [P] [US1] **PIVOTED. (No BootScene equivalent in Three.js; resource preloading can be done lazily inside the renderer; this task is dropped for this slice.)** N/A.
- [~] T025 [P] [US1] **PIVOTED.** Implement the start screen as a DOM overlay (`<div id="start-screen">` in `index.html`, styled in the inline CSS). The game loop in T026 hides it on the first key / pointer event. No separate "scene" class. File: handled in `index.html` + `src/game/game-loop.ts`.
- [~] T026 [US1] **PIVOTED.** Implement `src/game/game-loop.ts`: instantiates `createPlayerState`, `createWorldState`, `createInputAdapter({ now: performance.now, emit })`, `createThreeRenderer(canvas)`, `createDebugOverlay(debugOverlayEl)`. Owns the scene-state machine (`'start-screen' | 'running' | 'paused'`). Bridges DOM `keydown` + `pointerdown` + `pointerup` events into the input-adapter; the first input in `'start-screen'` state hides the start overlay and transitions to `'running'`. Runs `requestAnimationFrame(loop)` where loop computes `dtMs` (capped 100 ms), and when `'running'` calls `tickPlayer`, `tickWorld`, `renderer.draw`, `debugOverlay.update`. Wires `document.visibilitychange` + `window.blur` -> `pauseRun` + show pause overlay; on visible/focus, set `isAwaitingResume = true` and require any input to resume (consumes the resume input, does not let it also trigger a lane change). Wires `window.resize` -> `renderer.resize`. File: `src/game/game-loop.ts`.
- [~] T027 [US1] **PIVOTED.** Implement `src/main.ts`: resolves canvas + overlay DOM elements by id, then calls `createGameLoop({ canvas, startScreen, pauseOverlay, debugOverlay })`. File: `src/main.ts`.
