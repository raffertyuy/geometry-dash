...
- [X] T028 [US2] Manual desktop validation per [quickstart.md](./quickstart.md) §4-5: hit Medium and Advanced gates; verify clear SVG diagrams render in the modal with labels matching the prompt. Hit Basic gates; verify the figure slot stays empty (CSS `:empty` hides it).
...
- [X] T037 [US3] Manual desktop validation: open `localhost:5173`; on the start screen, click "Problem credits"; verify the panel opens with all sources; close with Escape; reopen; close with click-outside; reopen; close with the close button (if implemented). Trigger a game-over (lose all 3 lives); verify the link is also visible there; same dismissal paths work.
