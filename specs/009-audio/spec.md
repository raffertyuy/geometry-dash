...
- **FR-008** *(revised post-ship)*: The mute state MUST persist across run boundaries within the same browser session AND across page reloads (via client-side `localStorage`). Default on a brand-new origin (no stored preference) is un-muted.
