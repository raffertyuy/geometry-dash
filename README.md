...
|---|---|
| `npm run dev` | Vite dev server with hot module reload — runs the Worker locally too via `@cloudflare/vite-plugin`, so `/api/leaderboard` and the rest of the game both work against an in-memory KV stub. |
| `npm run dev:worker` | `wrangler dev` directly — useful when you want to test against the *built* `dist/` instead of the live HMR source. Requires `npm run build` first. |
| `npm run deploy` | Build + `wrangler deploy`. Pushes the Worker + static assets to Cloudflare. |
