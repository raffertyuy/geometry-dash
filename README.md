...
- **Audio (slice 009)**: a looping default BGM during running, plus a tense math-contest theme that swaps in while a problem-gate modal is open (and swaps back when you close it). Procedural SFX (built with OscillatorNodes + gain envelopes — zero asset bytes) for lane changes, obstacle hits, problem-cube hits, correct answers, life losses, the countdown's last 10 seconds, and game-over. A HUD mute button (top-right) or the `M` key silences everything in one tap; mute is a master-gain switch so un-muting picks up exactly where the music would have been. First user gesture unlocks audio so mobile autoplay is respected; pause-button / blur fully suspends audio. Two BGM placeholders ship in `public/audio/` (~520 KB combined); they're synthesised loops awaiting real CC0 / CC-BY 4.0 replacements (sourcing guidance in `public/audio/README.md`).
