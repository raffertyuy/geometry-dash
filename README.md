...
- **Single-penalty rule for wrong answers**: a wrong / timed-out problem answer that drives the running score below zero now ends the run **without** also deducting a life — the score-below-zero game-over is the one penalty, not "lose a life AND end the run". When the wrong answer leaves the score non-negative, the life cost is unchanged.
