...
- **Reliable life-deduction on wrong answers**: every wrong / timed-out problem answer costs exactly one life, even when the resulting score drops below zero and ends the run on the same tick. The lives HUD is pushed eagerly on the answer-resolve callback so the player always sees the deduction (3 → 2 hearts) before the game-over screen takes over.
